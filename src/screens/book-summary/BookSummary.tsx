...
import { postBookSummary } from "@/lib/axios/api";
...
function BookSummary() {
  ...
  const { mutate } = useMutation({
    mutationFn: () => postBookSummary({ content: text }),
  ...
  const handleSubmitBookSummary = (e: any) => {
    e.preventDefault();
    if (text === "") {
      toast({
        variant: "destructive",
  ...
  return (
    <div className="w-full">
      <form
        ...
        onSubmit={handleSubmitBookSummary}>
        <div className="flex w-full lg:w-2/3 items-center justify-center rounded-full shadow-lg">
          <TodaiInput
            ...
            type="text"
            placeholder="Please insert the name of the book you want to summarise and learn about"
            extra="w-full"
          ...
          />
          <TodaiAnimatedButton
            onClick={handleSubmitBookSummary}
            type="button"
            variant="primary"
