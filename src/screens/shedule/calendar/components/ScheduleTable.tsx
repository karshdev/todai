...
const ScheduleTable = ({ timeSlots, setTimeSlots, updateTimeSlot }: any) => {
                    ...
                    <TableRow key={slotIndex}>
                        <TableCell className='w-full'>
                            <Flatpickr
                                ...
                                onChange={(selectedDates, dateStr) => handleChangeTime(slotIndex, selectedDates, dateStr)}
                        ...
                        {daysOfWeek.map((day, dayIndex) => (
                            <TableCell key={day} className="text-center">
                                <Checkbox
                                    checked={slot.days.some((d: any) => d.day.id === dayIndex + 1)}
                                    onCheckedChange={() => handleCheckboxChange(slotIndex, dayIndex)}
                                    className="mx-auto px-auto text-brand-primary"
                                />
                            </TableCell>
                        ))}
                        <TableCell className="text-center">
                            <Button variant="ghost" size="icon" className={`${slotIndex == 0 && 'hidden'}`} onClick={() => handleDeleteSlot(slotIndex)}>
