...
export const signInUser = async (credentials: SignInProp) => {
  try {
    console.log("credentials", BASE_URL, " ", SIGNIN_URL);
...
export const getProfileData1 = async () => {
  //not passing token
  try {
    const url = `${BASE_URL}${GET_PROFILE_DATA_URL}`;
    const response = await getDataWithToken(url);
    console.log(response, "response of api to get routes");
    return response;
  } catch (error) {
    throw error;
  }
...
export const getLinkedInPosts = async () => {
  try {
    ...
    const interestIds = interestResponse?.data?.data?.interests
...
export const fetchWorldNews = async () => {
  try {
    ...
    const interestIds: number[] = interestResponse?.data?.data?.interests.map(
      (item: any) => item.id
    );
...
export const fetchCaptionAndHashtags = async (fileInput: File | string) => {
  try {
    ...
    const url = `${BASE_URL}${BYOI_IMAGE_TO_CAPTION}`;
    const response = await postDataWithToken(url, formData, true);
    if (response.status !== 200) {
      throw new Error("Failed to Upload Resource");
    }
