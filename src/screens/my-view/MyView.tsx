...
import { postMyView } from "@/lib/axios/api";
...
function MyView() {
  ...
  const { mutate } = useMutation({
    mutationFn: () => postMyView({ content: text }),
  ...
  const handleSubmitMyView = (e: any) => {
    e.preventDefault();
    if (text === "") {
      toast({
        variant: "destructive",
      ...
      <form
        ...
        <div className="flex w-full lg:w-2/3 items-center justify-center rounded-full shadow-lg">
          <TodaiInput
            ...
            type="text"
            placeholder="Eg: Give me a view on the latest Jaguar rebrand"
            extra="w-full"
